...
public class Ring {
    ...
    void initElection(int process_id) {
        ...
        int curr = process_id-1;  //index
        curr++;

        while(curr!=process_id-1){
            if(proc[curr]==true){
                pid.add(curr+1);
                System.out.print("Process P" + (curr+1) + " sending the following list:- ");
                displayArrayList(pid);
            }
            curr = (curr+1)%max_processes;
        }

        int coordinator = Collections.max(pid);
